...
const App = () => {
   ...
   * Specifically handles "d/m/Y" (e.g. 11/05/2026) from WordPress ACF
   */
  const formatDate = (dateStr) => {
    ...
    try {
      ...
      // Logic for d/m/Y format (Day/Month/Year)
      if (dateStr.includes('/')) {
      ...
      // Fallback for YYYYMMDD (Internal ACF default)
  ...
  const Navbar = () => (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#F5A623]/10 px-4 md:px-8 h-20 flex items-center justify-between shadow-sm">
      ...
      <div className="flex items-center space-x-4">
        <button 
          onClick={toggleLang} 
          className="flex items-center space-x-2 px-4 py-2 rounded-full border border-[#F5A623]/20 hover:bg-[#FFFCF8] text-[#2D2422] font-bold text-xs uppercase transition-all shadow-sm"
        >
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-80 text-[#F5A623]">
          ...
          <p className="font-serif text-sm text-[#2D2422] opacity-40 tracking-[0.2em] uppercase font-bold text-center">
            {lang === 'en' ? 'Synchronizing with Stars...' : 'सितारों के साथ तालमेल बिठा रहे हैं...'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-10">
            {festivals.map(festival => (
              <div 
                ...
                className="bg-white rounded-[3.5rem] p-5 shadow-sm border border-[#F5A623]/5 hover:shadow-2xl hover:-translate-y-2 transition-all duration-700 cursor-pointer group flex flex-col"
              >
                <div className="relative h-64 rounded-[2.5rem] overflow-hidden mb-6 bg-gray-50">
                  ...
                  />
                  <div className="absolute top-5 left-5 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full text-[9px] font-bold text-[#DF4832] uppercase tracking-[0.3em] shadow-sm">
                    {festival.type}
                  </div>
                </div>
                <div className="px-3 flex-grow flex flex-col">
                  ...
                  <button className="mt-auto w-full py-4 rounded-[1.5rem] bg-[#FFFCF8] text-[#DF4832] font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-[#F5A623] hover:text-white transition-all shadow-sm">
                    {lang === 'en' ? 'Enter Story' : 'कहानी में प्रवेश करें'}
                  </button>
                </div>
              </div>
            ))}
          ...
          <aside className="lg:col-span-3 space-y-12">
            ...
            <PromoWidget 
              category={lang === 'en' ? "Kids Learning" : "बच्चों की शिक्षा"}
              title={lang === 'en' ? "Dharma Puzzles" : "धर्म पहेलियाँ"}
              description={lang === 'en' ? "Engaging jigsaw puzzles teaching Vedic history to the next generation." : "अगली पीढ़ी को वैदिक इतिहास सिखाने वाली आकर्षक पहेलियाँ।"}
              image="https://images.unsplash.com/photo-1516979187457-637abb4f9353?auto=format&fit=crop&q=80&w=600"
              buttonText={lang === 'en' ? "Begin Now" : "अभी शुरू करें"}
            />
      ...
      <div className="min-h-screen bg-[#FFFCF8] pb-24 relative z-10">
        <div className="max-w-7xl mx-auto px-4 py-10">
          <button 
            ...
            className="flex items-center text-[#2D2422]/40 hover:text-[#DF4832] mb-12 bg-white px-8 py-3.5 rounded-full shadow-sm font-bold text-[10px] uppercase tracking-widest transition-all"
          >
            ...
            <article className="lg:col-span-8 space-y-16">
              ...
              <div className="relative rounded-[4.5rem] overflow-hidden shadow-2xl h-[450px] md:h-[650px] border-[8px] border-white">
                ...
                <div className="absolute bottom-0 left-0 p-10 md:p-20">
                  <span className="inline-block bg-[#DF4832] text-white px-6 py-2 rounded-full text-xs font-bold uppercase tracking-[0.3em] mb-8 shadow-2xl">
                    {selectedFestival.type}
                  </span>
                  <h1 className="font-serif text-6xl md:text-8xl text-white font-bold tracking-tighter leading-[0.85]">
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-12 rounded-[3.5rem] border border-[#F5A623]/20 shadow-sm flex flex-col items-center text-center group hover:border-[#F5A623] transition-colors">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#2D2422]/30 mb-3">
                    {lang === 'en' ? 'North Indian Date' : 'उत्तर भारतीय तिथि'}
                  </h3>
                ...
                <div className="bg-white p-12 rounded-[3.5rem] border border-[#DF4832]/20 shadow-sm flex flex-col items-center text-center group hover:border-[#DF4832] transition-colors">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[#2D2422]/30 mb-3">
                    {lang === 'en' ? 'South Indian Date' : 'दक्षिण भारतीय तिथि'}
                  </h3>
      ...
      <div className="max-w-7xl mx-auto px-4 text-center">
        <div className="flex flex-col items-center space-y-12">
          ...
          <div className="space-y-6">
            <p className="text-white/40 text-xs tracking-[0.6em] uppercase font-black">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans selection:bg-[#F5A623]/30 selection:text-[#2D2422]">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        ::-webkit-scrollbar-thumb { background: #F5A623; border-radius: 6px; border: 3px solid #FFFCF8; }
